...
account_url: str = '/api/v1/accounts/delete/'
...
@pytest.mark.anyio
async def test_delete_account_failed_invalid_access_token(client: httpx.AsyncClient):
    # Header
    test_header = {
    ...
    # Send request
    response = await client.delete(account_url, params=test_body, headers=test_header)

    assert response.status_code == status.HTTP_401_UNAUTHORIZED


@pytest.mark.anyio
async def test_delete_account_failed_not_admin(client: httpx.AsyncClient):
    ...
    if status_code < status.HTTP_200_OK:
        ...
        # Send request
        response = await client.delete(account_url, params=test_reset_phone_number_body, headers=test_auth_header)

        assert response.status_code == status.HTTP_403_FORBIDDEN


@pytest.mark.anyio
async def test_delete_account_failed_invalid_account_id(client: httpx.AsyncClient):
