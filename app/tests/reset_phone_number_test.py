...
account_url: str = '/api/v1/accounts/reset-phone-number/'
...
@pytest.mark.anyio
async def test_reset_phone_number_failed_invalid_access_token(client: httpx.AsyncClient):
    # Header
    test_header = {
    ...
    # Send request
    response = await client.post(account_url, params=test_reset_phone_number_body, headers=test_header)

    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.anyio
async def test_reset_phone_number_successful(client: httpx.AsyncClient):
